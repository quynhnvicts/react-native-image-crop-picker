...
- (void)getVideoAsset:(PHAsset*)forAsset completion:(void (^)(NSDictionary* image))completion {
    ...
    if (@available(iOS 11.0, *)) {
        [manager requestExportSessionForVideo:forAsset options:options exportPreset:AVAssetExportPresetMediumQuality resultHandler:^(AVAssetExportSession * _Nullable exportSession, NSDictionary * _Nullable info) {
            
            exportSession.outputURL = outputURL;
            exportSession.outputFileType = AVFileTypeMPEG4;
            [self.compression compressWithExportSession:exportSession withOptions:self.options handler:^(AVAssetExportSession *exSession) {
                if (exSession.status == AVAssetExportSessionStatusCompleted) {
                    ...
                    NSURL *sourceURL = [(AVURLAsset *)exSession.asset URL];
